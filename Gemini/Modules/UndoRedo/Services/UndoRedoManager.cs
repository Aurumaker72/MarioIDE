...
namespace Gemini.Modules.UndoRedo.Services
{
    public class UndoRedoManager : PropertyChangedBase, IUndoRedoManager
    {
        ...
        private void EnforceLimit()
        {
            ...
            int removeCount = ActionStack.Count - UndoCountLimit.Value;
            if (removeCount <= 0)
                return;
        ...
        public void ExecuteAction(IUndoableAction action)
        {
            if (UndoActionCount < ActionStack.Count)
            {
                ...
                for (int i = ActionStack.Count - 1; i >= UndoActionCount; i--)
        ...
        public void UndoAll()
        {
            ...
            for (int i = UndoActionCount - 1; i >= 0; i--)
                ActionStack[i].Undo();
